...
# Parameters for sampling and partitioning
...
def greedy_matching(partition):
    matched = set()  # Track matched vertices
    matching = set()  # Track the matched edges
    for edge in partition:
        u, v = edge  # Unpack edge into u and v
        if u not in matched and v not in matched:  # Only match if both vertices are unmatched
            matching.add((u, v))  # Add edge to matching
            matched.add(u)  # Mark vertex u as matched
            matched.add(v)  # Mark vertex v as matched
...
final_matching_clean.saveAsTextFile("hdfs://hadoop-namenode:9820/project/facebook_matching_result89")
